...
namespace CleanStartup.Swagger
{
    public class AddEnumDescriptions : IDocumentFilter
    {
        public void Apply(OpenApiDocument swaggerDoc, DocumentFilterContext context)
        {
            ...
            foreach (KeyValuePair<string, OpenApiSchema> property in swaggerDoc.Components.Schemas.Where(x => x.Value?.Enum?.Count > 0))
            {
        ...
        private static void DescribeEnumParameters(IDictionary<OperationType, OpenApiOperation> operations, OpenApiDocument swaggerDoc)
        {
            if (operations != null)
            {
                foreach (KeyValuePair<OperationType, OpenApiOperation> oper in operations)
                {
                    foreach (OpenApiParameter? param in oper.Value.Parameters)
                    {
                        KeyValuePair<string, OpenApiSchema> paramEnum = swaggerDoc.Components.Schemas.FirstOrDefault(x => x.Key.ToLower() == param.Name.ToLower());
